...
@app.route('/maiores_altas', methods=['GET'])
def obter_dados():
    ...
    if response.status_code == 200:
        ...
        if input_element:
            ...
            # Ordena os dados com base no campo 'resultPercentageValue' em ordem crescente
            dados.sort(key=lambda x: x['resultPercentageValue'])

            # Seleciona apenas os três últimos elementos da lista
            output_data = [{'afterMarket': item['afterMarket'], 'code': item['code'], 'resultPercentageValue': item['resultPercentageValue']} for item in dados[-3:]]
